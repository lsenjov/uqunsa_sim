...
The `home-routes` handler in the `uqunsa-sim.routes.home` namespace
...
The `home-page` function will in turn call the `uqunsa-sim.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/uqunsa_sim/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `uqunsa-sim.handler` namespace:
...
Request middleware functions are located under the `uqunsa_sim.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `uqunsa-sim.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `uqunsa-sim.db.core` in some other namespace.
